...
<html>
<head>
	<title>COOU Medical Centre|View Complain</title>
...
$per_page = 5;
$pages_query = mysql_query("SELECT COUNT('id') FROM complain");
...
$query = "SELECT * FROM complain LIMIT $start,$per_page";
...
<div class="panel-heading" style="background:darkblue;"><h3>Students Complain</h3></div>
...
<?php
	if(!($page<=1)){
	echo "<a href='ViewComplain.php?page=$prev'>Prev</a> ";
	}
	if($pages >= 1){
		for ($i=1; $i <=$pages ; $i++) { 
	...
	if(!($page>=$pages)){
	echo " <a href='ViewComplain.php?page=$next'>Next</a>";
	}
?>
<br>
